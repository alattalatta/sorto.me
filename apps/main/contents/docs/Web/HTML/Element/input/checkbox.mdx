...
`checkbox` 유형도 [유효성 검증](https://developer.mozilla.org/en-US/docs/Web/Guide/HTML/Constraint_validation)을 지원하기는 하지만, 대부분의 [`ValidityState`](https://developer.mozilla.org/en-US/docs/Web/API/ValidityState) 속성은 항상 `false`입니다. 다만, [`required` 특성](../input#term-required)을 지정한 체크박스를 체크하지 않은 상태에서는 `ValidityState.valueMissing`이 `true`입니다.
