...
`<input type="checkbox">` 요소의 [`value` 특성](../input#term-value)은 체크박스의 값을 나타내는 문자열이어야 합니다. 이 값은 클라이언트에는 노출되지 않고, 양식을 제출할 때 체크박스의 [`name`](../input#term-name)과 함께 서버로 전송됩니다. 아래의 예제 코드를 봐주세요.
