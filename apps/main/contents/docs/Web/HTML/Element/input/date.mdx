...
[`required` 특성](../input#term-required)으로 날짜를 반드시 입력하게 설정할 수 있습니다. 사용자가 날짜를 입력하지 않고 양식을 제출하려고 하면 브라우저가 오류를 표시합니다. 필수 입력 설정은 `datetime-local` 입력 칸이 텍스트 입력 칸으로 대체된 경우에도 동작합니다.
