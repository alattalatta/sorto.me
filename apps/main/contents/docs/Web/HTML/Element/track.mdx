...
**HTML `<track>` 요소**는 [`<audio>`](./audio)와 [`<video>`](./video)와 같은 미디어 요소의 자식으로 사용해서, 자막 등의 시간 기반 텍스트 트랙을 지정할 때 사용합니다.
...
- : `src`
  - 삽입할 트랙(`.vtt 파일`)의 URL입니다. 필수 특성입니다. 이 `<track>`의 부모 `<audio>` 또는 `<video>` 요소에 [`crossorigin` 특성](./audio#term-crossorigin)이 존재하지 않는다면 `src` 특성에 사용할 URL의 출처는 현재 문서의 출처와 동일해야 합니다.
- : `srclang`
  - 텍스트 트랙의 언어를 지정합니다. [전역 `lang` 특성](../Global_attributes#term-lang)과 같은 값을 사용합니다. [`kind` 특성](#term-kind)이 `subtitles`인 경우에는 필수 특성입니다.
