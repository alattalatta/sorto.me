...
- : `<number>`
  - 1 이상 1000 이하의 [`<number>`](./number) 값입니다. 높은 값은 더 굵은 글꼴, 낮은 값은 더 얇은 글꼴을 나타냅니다. 가변 글꼴에서는 정확한 수치를 입력해 세부 조정이 가능합니다. 비가변 글꼴에서는, 지정한 정확한 값에 해당하는 굵기가 없다면 [굵기 대체 알고리즘](#대체-굵기)을 사용합니다. 100 단위로 끊어지는 값들에는 자주 쓰이는 별칭이 있습니다. 아래의 [자주 쓰이는 굵기 이름](#자주-쓰이는-굵기-이름)에서 확인하세요.
- : `lighter`
