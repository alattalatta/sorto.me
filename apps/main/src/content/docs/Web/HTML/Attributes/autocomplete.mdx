...
**HTML `autocomplete` 특성**은 사용자 에이전트가 웹 페이지의 양식 컨트롤 데이터를 자동으로 채울 수 있도록 지정하고, 어떤 종류의 데이터를 사용해야 하는지에 대한 도움말을 제공합니다.
...
`autocomplete` 특성의 값으로 토큰 리스트를 사용했고, 동일한 리스트가 두 개 이상의 양식 컨트롤에 동일하게 쓰인 경우, 사용자 에이전트는 해당하는 모든 요소들을 같은 데이터로 자동완성합니다.
...
- : `<token-list>`
  ...
  - 세부 자동완성 토큰들의 정렬된 공백 구분 리스트입니다. 사용 가능한 토큰들은 다음과 같습니다.

    - : `section-*`
      - 양식 컨트롤들의 유명 그룹 이름을 정의합니다. 첫 8글자는 대소문자 구분 없는 "section-"이어야 하고, 그 뒤를 그룹의 이름으로 잇습니다. `section-*` 토큰은 항상 토큰 리스트의 제일 첫 항목이어야 합니다. 같은 `section-*` 토큰을 가진 양식 컨트롤들은 같은 그룹에 속하게 됩니다. `"section-product1 shipping street-address"`, `"section-product2 shipping street-address"`처럼 사용할 수 있습니다.
    - : `name`
    ...
    - : `street-address`
      ...
      - 도로 주소입니다. 2차 행정구역 (시/군/구 등) 내에서 온전히 식별 가능한 주소여야 하지만 도시명, 우편번호, 국가명은 포함하지 않아야 합니다.
        ...
        - : `shipping`
          - 상품을 보내야 하는 주소입니다. `shipping street-address`와 같이 사용합니다. `shipping address-level2`처럼 다른 토큰과도 조합할 수 있습니다.
        - : `billing`
          - 청구지 주소입니다. `billing street-address`와 같이 사용합니다. `billing address-level2`처럼 다른 토큰과도 조합할 수 있습니다.
