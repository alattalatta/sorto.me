...
<LiveExample name="range" height={100} />
...
단순히 현재 사용자의 시간대를 제출하기만 하면 되는 경우에는 [`hidden` 입력 칸](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/input/hidden)에 적절한 값을 지정하면 됩니다. 제출 버튼을 눌러보세요.

```html hidden name=timezone
<form>
...
<LiveExample name="timezone" height={180} />
