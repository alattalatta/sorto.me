...
`<portal>`의 기본 레이블은 삽입한 페이지의 제목입니다. 제목이 따로 존재하지 않는다면 보여지는 영역의 텍스트를 이어붙여 레이블을 생성합니다. [`aria-label`](https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/Attributes/aria-label) 특성으로 기본 레이블을 재정의할 수 있습니다.

프리렌더링만을 위해 사용한 `<portal>`은 `hidden` 전역 특성 또는 CSS [`display: none`](/docs/Web/CSS/display)을 사용해 숨겨야 합니다.
