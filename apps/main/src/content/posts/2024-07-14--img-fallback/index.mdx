...
상품 썸네일처럼 중요한 이미지를 불러오지 못했을 때, 그 자리를 비워놓거나 *엑박*으로 방치하지 않고 **대체 이미지**를 대신 노출해달라는 건 기본적인 요구사항이다. 그러나 그 구현은 쉽지 않다.
...
`loading="lazy"`일 땐 스크립트보다 이미지 로딩이 느릴테니 `onError`를 쓰고, 아닐 땐 지금처럼 `new Image()`를 하면 어떨까? 하지만 고작 *엑박*을 대체하고 싶을 뿐인데... `loading="lazy"`인 이미지가 처음부터 뷰포트 내에 존재했을 때도 스크립트보다 느릴지도 확실하지 않다.
...
Chromium 브라우저 또는 Safari로 접속 후, 위 예제의 `image/png`를 참고하라. 제대로 보이지 않는다. 원래 요청하려던 이미지가 PNG면 대체 이미지도 반드시 PNG여야 하고, 원래 SVG를 원했다면 대체 이미지도 SVG여야 한다. 교차해서 사용할 수 없다.
...
그리고 `<img>`는 아마도 대체되는 요소, 즉 [대체 요소](/docs/Web/CSS/Replaced_element)다. 대체 요소 `<img>`에 의사 요소를 넣고도 무시되지 않겠다는 것의 근거는 HTML 명세에서 찾을 수 있다.

<blockquote>

사용자 에이전트는 `<img>` 요소를 렌더링할 때 다음 목록에서 제일 처음 만족하는 규칙에 따르는 것이 예상된다.

- : 해당 요소가 이미지를 표현한다면

  - 사용자 에이전트는 해당 요소를 대체 요소로 취급하고 CSS에서 정의된 바에 따라 이미지를 렌더링 하는 것이 예상된다.

- : 해당 요소가 이미지를 표현하지 않고, 다음 중 하나라면

  - - 사용자 에이전트가 이미지를 곧 사용 가능하고 따라서 렌더링할 수 있을 거라고 믿는 이유가 있거나
    - 요소에 `alt` 특성이 없음

    사용자 에이전트는 해당 요소를 대체 요소로, 그 내용은 요소가 표현하는 텍스트가 존재한다면 그것으로 취급하는 것이 예상된다. 적용 가능하다면, 이미지를 불러오고 있음을 나타내는 아이콘을 선택적으로 표시할 수 있다.
...
Chromium 계열 브라우저에서는 위 이미지 세 개 모두 대체 이미지로 노출되는 걸 확인할 수 있다. 다른 브라우저에서도 동일했으면 좋았겠지만, 이 동작은 표준에서 살짝 벗어나는 동작이므로 기대할 수 없다. 잘 보면 세 번째 이미지에서 차이점이 보인다.

명세에 따르면, `<img>`가 이미지를 표현하지 않고 요소에 `alt` 특성이 없다면 대체되어야 한다. 따라서 의사 요소가 무시돼야 한다. 그런데 세 번째 이미지, `<img src="https://example.org/void.png">`도 의사 요소에 의해 대체 이미지로 나타나고 있다.

또한 `<img>`가 아무것도 표현하지 않는다면 대체되어야 하는데, 두 번째 이미지의 `alt`는 비어있고 이미지 또한 오류다. 관점에 따라 이 `<img>`는 아무것도 표현하지 않는 것일 수 있다. 이에 따라 Firefox는 두 번째 이미지의 의사 요소를 무시하여 빈 상자로 나타난다. 하지만 Chromium 브라우저에서는 대체 이미지로 나타난다.
