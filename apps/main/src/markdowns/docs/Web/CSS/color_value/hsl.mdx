...
**`hsl()`** 함수형 표기는 [sRGB](https://ko.wikipedia.org/wiki/SRGB) 색을 색상(_hue_), 채도(_saturation_), 명도(_lightness_) 성분으로 표현합니다. 선택적으로 알파 성분이 색의 투명도를 나타냅니다.
...
`hsl()` 색의 색상 성분에 180°를 더하거나 빼서 보색을 구할 수 있습니다. 예를 들어, 어느 색의 색상 성분이 `10deg`라면, 보색은 `190deg`가 됩니다.
