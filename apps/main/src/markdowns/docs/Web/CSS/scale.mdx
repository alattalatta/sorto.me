---
title: 'CSS scale: 크기 변형'
titleShort: 'scale'
bcd: css.properties.scale
description: 'CSS scale 속성은 transform 속성과 별개로 요소에 크기 변형을 적용합니다.'
---

CSS **`scale`** 속성은 [`transform` 속성][`transform`]과 별개로 요소에 크기 변형을 적용합니다. 이 속성은 일반적인 사용자 인터페이스 변형에 더 적합하고, `transform` 속성에서 변형 함수들이 적용되는 순서를 기억하지 않아도 되므로 편리합니다.
