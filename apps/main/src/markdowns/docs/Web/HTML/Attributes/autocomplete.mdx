...
[`<input>`], [`<select>`], [`<textarea>`] 요소에 `autocomplete` 특성이 없으면 브라우저는 해당 요소들의 **양식 소유자**가 가진 `autocomplete` 특성을 대신 사용합니다. 양식 소유자란, 요소에 [`form` 특성](../Element/input#term-form)을 지정한 경우 그 값과 일치하는 [`id`](../Global_attributes/id)의 [`<form>`] 요소이고, 아닐 경우 양식 요소들의 부모 또는 조상 `<form>` 요소입니다.
...
- : `url`
  - 홈페이지나 회사 웹 사이트 등 양식의 맥락에 맞는 URL입니다.
- : `photo`
