...
`datetime-local` 유형의 값은 [유효한 HTML 현지 날짜 및 시간 문자열](https://developer.mozilla.org/en-US/docs/Web/HTML/Date_and_time_formats#local_date_and_time_strings)입니다.

사용자의 화면에 나타나는 날짜 및 시간의 형식은 `datetime-local` 입력 칸이 실제로 저장하는 값과는 다른 형식을 사용합니다. 사용자에게 보이는 날짜 및 시간 형식은 브라우저의 로케일에 따라 달라지지만, 실제 저장 값과 [`value` 특성](../input#term-value)은 항상 [유효한 HTML 현지 날짜 및 시간 문자열](https://developer.mozilla.org/en-US/docs/Web/HTML/Date_and_time_formats#local_date_and_time_strings) 형식 중 하나를 사용합니다. 예를 들어, 문서 처음의 예제에서 사용자는 "2077-06-15 오후 01:27"이나 이와 비슷한 형식의 글을 볼 수 있지만, 실제 `value`는 `2077-06-15T13:27`일 것입니다.
