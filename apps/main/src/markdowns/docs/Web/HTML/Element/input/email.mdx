...
`email` 유형의 값은 문자열입니다. [`value` 특성](../input#term-value)과 입력 값 모두 이메일 주소 구문에 맞는지 [유효성 검증](#유효성-검증)을 거칩니다. 이메일 주소 구문 검증을 통과할 수 있는 값에는 다음과 같은 세 종류가 있습니다.
...
`email` 유형의 검증에는 두 종류가 있는데, 그중 첫 번째는 값이 유효한 형식의 이메일 주소임을 검증하는 것이고 두 번째는 이메일 주소 중에서도 추가 조건에 맞는 주소를 검증하는 패턴 검증입니다.
...
"아무 이메일"보다 정교한 제한이 필요하면, 값이 일치해야 하는 정규표현식을 [`pattern` 특성](#term-pattern)에 지정할 수 있습니다. [`multiple` 특성](#term-multiple)을 지정했으면 쉼표로 구분한 각각의 이메일이 모두 정규표현식과 일치해야 합니다.
