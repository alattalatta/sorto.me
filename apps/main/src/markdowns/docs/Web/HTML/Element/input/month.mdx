...
`month` 유형의 값은 [유효한 HTML 연/월 문자열](https://developer.mozilla.org/en-US/docs/Web/HTML/Date_and_time_formats#month_strings)입니다.

사용자의 화면에 나타나는 날짜의 형식은 `month` 입력 칸이 실제로 저장하는 값과는 다른 형식을 사용합니다. 사용자에게 보이는 날짜 형식은 브라우저의 로케일에 따라 달라지지만, 실제 저장 값과 [`value` 특성](../input#term-value)은 항상 `YYYY-MM` 형식을 사용합니다.
