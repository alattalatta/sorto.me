...
> [note] > `transparent`를 [`<gradient>`](https://developer.mozilla.org/en-US/docs/Web/CSS/gradient)를 포함한 위치에서 사용했을 때 예상하기 힘든 결과를 낳는 일을 방지하기 위해, 현 CSS 명세는 `transparent` 색을 계산할 때 [알파 채널을 곱한 색 공간](https://www.w3.org/TR/css-color-4/#interpolation-alpha) 위에서 계산하도록 명시하고 있습니다. 그러나 구형 브라우저에서는 `transparent`가 알파 값 0의 검은 색으로 나타날 수 있는 점을 주의하세요.
...
<LiveExample height={120} light name="currentColor" />
...
- : 함수 표기법: `rgb(R, G, B)`, `rgba(R, G, B)`, `rgb(R, G, B, A)`, `rgba(R, G, B, A)`
  ...
  - `R`(적), `G`(녹), `B`(청)를 [`<number>`](./number) 또는 [`<percentage>`](https://developer.mozilla.org/en-US/docs/Web/CSS/percentage)로 표기하며, 숫자 `255`는 `100%`와 같습니다. `A`(알파)는 `<percentage>` 또는 0 이상 1 이하의 `<number>`로 숫자 `1`은 `100%`(불투명)와 같습니다.
...
- : 함수 표기법: `hsl(H, S, L)`, `hsla(H, S, L)`, `hsl(H, S, L, A)`, `hsla(H, S, L, A)`
  ...
  - `H`(색상)는 색상환 위의 각도입니다. [CSS Color Module Level 4](https://drafts.csswg.org/css-color/#the-hsl-notation) 명세에 따라 `deg`, `rad`, `grad`, `turn` 단위를 사용한 [`<angle>`](https://developer.mozilla.org/en-US/docs/Web/CSS/angle), 또는 [CSS Color Module Level 3](https://drafts.csswg.org/css-color-3/#hsl-color) 명세에 따라 단위 없는 [`<number>`](./number)를 지정할 수 있으며, `<number>`를 사용하면 도 단위(`deg`)로 해석합니다. 색상환의 정의에 따르면 적색=`0deg`=`360deg`고, 다른 모든 색은 원 위에 균일하게 위치합니다. 예컨대 녹색=`120deg`, 청색=`240deg`입니다. `<angle>` 자료형이므로 0도 ~ 360도를 벗어나면 원을 한 바퀴 돌게 됩니다. 따라서 `-120deg`=`240deg`, `480deg`=`120deg`, `-1turn`=`1turn`입니다.
    ...
    `S`(채도)와 `L`(명도)은 [`<percentage>`](https://developer.mozilla.org/en-US/docs/Web/CSS/percentage)입니다. **채도** `100%`는 완전한 유채색, `0%`는 완전한 무채색(회색)입니다. **명도** `100%`는 백색, `0%`는 흑색, `50%`는 중간 회색입니다.
...
HSL과 마찬가지로 RGB보다 HWB를 더 직관적으로 받아들이기 쉽습니다. 색상 매개변수는 HSL과 동일하고, 그 뒤에 백색과 흑색 매개변수를 [`<percentage>`](https://developer.mozilla.org/en-US/docs/Web/CSS/percentage) 값으로 제공해야 합니다. 선택적으로 알파 값도 지정할 수 있습니다.
...
- : 함수 표기법: `hwb(H W B)`, `hwb(H W B/A)`
  ...
  - `H`(색상)는 색상환 위의 각도입니다. [CSS Color Module Level 4](https://drafts.csswg.org/css-color/#the-hsl-notation) 명세에 따라 `deg`, `rad`, `grad`, `turn` 단위를 사용한 [`<angle>`](https://developer.mozilla.org/en-US/docs/Web/CSS/angle), 또는 [CSS Color Module Level 3](https://drafts.csswg.org/css-color-3/#hsl-color) 명세에 따라 단위 없는 [`<number>`](./number)를 지정할 수 있으며, `<number>`를 사용하면 도 단위(`deg`)로 해석합니다. 색상환의 정의에 따르면 적색=`0deg`=`360deg`고, 다른 모든 색은 원 위에 균일하게 위치합니다. 예컨대 녹색=`120deg`, 청색=`240deg`입니다. `<angle>` 자료형이므로 0도 ~ 360도를 벗어나면 원을 한 바퀴 돌게 됩니다. 따라서 `-120deg`=`240deg`, `480deg`=`120deg`, `-1turn`=`1turn`입니다.
    ...
    `W`(백색)와 `B`(흑색)는 [`<percentage>`](https://developer.mozilla.org/en-US/docs/Web/CSS/percentage)입니다. 두 값을 혼합하면 최종 색이 되므로 완전한 검은색은 **흑색** `100%` 뿐만 아니라 **백색** `0%`도 필요합니다. 완전한 흰색 또한 흑색 `0%`와 백색 `100%`가 필요합니다. 흑색과 백색을 둘 다 `50%`로 지정하거나 둘 다 `100%`로 지정하면 중간 회색입니다.
...
CSS Color Module Level 4에서 [`color()`](https://developer.mozilla.org/en-US/docs/Web/CSS/color_value/color) 함수가 추가됐습니다. `color()` 함수는 사전에 정의된 색 공간 뿐만 아니라 [`@color-profile`](https://developer.mozilla.org/en-US/docs/Web/CSS/@color-profile) 규칙으로 정의한 사용자 정의 색 공간 위 색 또한 표현할 수 있습니다.

# 보간

애니메이션과 [그레이디언트](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Images/Using_CSS_gradients)에서, `<color>` 값의 보간은 적색, 청색, 녹색 값에 대해 독립적으로 이뤄집니다. 각각의 값은 부동소수점 실수로 보간됩니다. 갑자기 무채색이 나타나는 일을 방지하기 위해, 색 보간은 [알파 채널을 곱한 색 공간](https://www.w3.org/TR/css-color-4/#interpolation-alpha)에서 진행합니다. 애니메이션 내의 보간 속도는 [타이밍 함수](https://developer.mozilla.org/en-US/docs/Web/CSS/easing-function)가 결정합니다.
