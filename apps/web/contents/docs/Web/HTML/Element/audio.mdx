...
**HTML `<audio>` 요소**는 오디오 콘텐츠를 문서에 삽입할 때 사용합니다. `src` 특성과 [`<source>`](./source) 요소를 이용해서 하나 이상의 오디오 소스를 제공할 수 있고, 여러 개를 제공한 경우 브라우저가 제일 적합한 소스를 선택합니다. [`MediaStream`](https://developer.mozilla.org/en-US/docs/Web/API/MediaStream)을 사용해 미디어 스트림의 목적지로 지정할 수도 있습니다.
...
브라우저가 지원하는 파일 유형과 오디오 코덱은 서로 다릅니다. [`<source>`](./source) 요소를 추가해서 다수의 소스를 제공하면 브라우저가 이해할 수 있는 첫 번째 소스를 자동으로 사용하므로 지원 범위를 넓힐 수 있습니다.
