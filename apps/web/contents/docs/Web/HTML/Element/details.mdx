...
description: HTML <details> 요소는 열기 전까지 내용을 숨기는 공개 위젯을 생성합니다. 내용의 요약, 또는 레이블을 <summary> 요소로 제공해야 합니다.
bcd: html.elements.details
---

**HTML `<details>` 요소**는 열기 전까지 내용을 숨기는 *공개 위젯*을 생성합니다. 내용의 요약, 또는 레이블을 [`<summary>`](./summary) 요소로 제공해야 합니다.
...
표준을 완전하게 준수하는 구현체에서는 `<summary>` 요소에 CSS [`display: list-item`](https://developer.mozilla.org/en-US/docs/Web/CSS/display)을 적용하고, [`list-style-type: disclosure-open`](https://developer.mozilla.org/en-US/docs/Web/CSS/list-style-type)(또는 `disclosure-close`)으로 삼각형을 표시합니다. 아래의 [CSS 스타일링 예제](#CSS-스타일링)에서 CSS 적용법에 대해 더 알아보세요.
...
<LiveExample height={160} name="open" />

## CSS 스타일링

시스템 요구사항 예제에 스타일을 적용해서 탭 인터페이스처럼 보이도록 하는 예제입니다.
...
[`[open]` 선택자](https://developer.mozilla.org/ko/docs/Web/CSS/Attribute_selectors)로 열린 상태의 `<details>`를 선택할 수 있습니다.
...
<LiveExample height={160} name="css" />

## 위젯 스타일링

위젯 레이블 옆의 삼각형도 CSS로 직접 바꿀 수 있지만, 많은 브라우저에서 지원하지는 않습니다. 정확한 지원 범위는 [CSS `list-style-type`의 브라우저 지원](https://developer.mozilla.org/en-US/docs/Web/CSS/list-style-type#browser_compatibility) 중 `disclosure-closed`와 `disclosure-open`을 참고하세요.

`<summary>` 요소는 [`list-style-type`](https://developer.mozilla.org/en-US/docs/Web/CSS/list-style-type), [`list-style-image`](https://developer.mozilla.org/en-US/docs/Web/CSS/list-style-image) 등을 포함해 [`list-style`](https://developer.mozilla.org/en-US/docs/Web/CSS/list-style) 단축속성을 지원합니다. 따라서 `<summary>`에 `list-style: none`을 적용하면 위젯에서 삼각형을 아예 제거할 수 있습니다.

### HTML

```html name=list-style
<details>
  <summary>시스템 요구사항</summary>
  <ul>
    <li>64비트 프로세서와 운영체제</li>
...
<LiveExample height={160} name="list-style" />
