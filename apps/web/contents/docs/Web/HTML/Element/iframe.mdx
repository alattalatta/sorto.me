...
> [warn]
> 각각의 브라우징 맥락은 완전히 분리된 환경입니다. 따라서 페이지 내의 `<iframe>`은 메모리를 포함한 자원을 추가로 요구합니다. 이론적으로는 `<iframe>`을 무한히 사용할 수 있지만, 성능 문제에 주의를 기울이세요.
...
`<iframe>`은 [대체 요소](https://developer.mozilla.org/ko/docs/Web/CSS/Replaced_element)로, CSS [`object-position`](https://developer.mozilla.org/en-US/docs/Web/CSS/object-position)과 [`object-fit`](https://developer.mozilla.org/en-US/docs/Web/CSS/object-fit) 속성을 사용해 삽입한 문서의 위치와 정렬, 크기를 조절할 수 있습니다.
