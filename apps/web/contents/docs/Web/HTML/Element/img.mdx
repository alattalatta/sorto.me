...
[`src` 특성](#term-src)은 [`srcset`](#term-srcset)과 `sizes`를 지원하지 않는 사용자 에이전트를 위해 지정해야 합니다. 모던 브라우저에서는 `srcset` 특성에 너비 서술자 `w`를 사용했으므로 `src`를 무시합니다.

# 접근성 고려사항

## 유의미한 대체 설명 작성

[`alt` 특성](#term-alt)의 값은 분명하고 간결하게 이미지의 내용을 설명해야 합니다. 이미지가 존재한다는 사실이나 파일 이름과 같은 값은 지정하지 말아야 합니다. 이미지에 대응 가능한 텍스트가 없어서 `alt`를 의도적으로 누락한 경우에는 이미지의 의도를 설명할 수 있는 다른 방법을 고려하세요.
...
일부 스크린 리더에서는 `alt`가 없으면 파일 이름을 대신 읽을 수도 있는데, 파일 이름은 이미지의 콘텐츠와 관련이 없을 수도 있기 때문에 사용자에게 혼란을 줄 수 있습니다. 되도록 항상 `alt`를 지정하세요.

## `title` 특성

[`title` 전역 특성](../Global_attributes#term-title)은 `alt` 특성을 대체할 수 없습니다. `alt`와 `title`의 값을 동일하게 설정하는 것도 피하세요. 일부 스크린 리더에서 같은 내용으로 이미지를 두 번 읽을 수 있습니다.
