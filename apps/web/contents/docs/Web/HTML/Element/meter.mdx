...
- : `form`
  ...
  - 이 `<meter>`를 _양식 소유자_([`<form>`](./form) 요소)와 연결합니다. 같은 문서 상에 존재하는 `<form>`의 [`id`](/docs/Web/HTML/Global_attributes#term-id) 값을 지정하세요. 이 특성이 존재하지 않으면 `<meter>`의 양식 소유자는 가장 가까운 조상 `<form>`이 됩니다.
