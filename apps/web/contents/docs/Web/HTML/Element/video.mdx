...
`<video>`는 [대체 요소](https://developer.mozilla.org/ko/docs/Web/CSS/Replaced_element)로, [`display`](https://developer.mozilla.org/en-US/docs/Web/CSS/display)의 기본값이 `inline`이지만 그 크기는 마치 `inline-block`인 것처럼 삽입한 비디오의 내재된 크기가 결정합니다.
