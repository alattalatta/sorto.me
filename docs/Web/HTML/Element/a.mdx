...
사용자를 연결된 URL로 바로 보내는 대신, 지정한 링크의 자원을 다운로드할지 여부를 물어보게 됩니다. 값을 지정할 수도 있고, 지정하지 않을 수도 있습니다.
...
- `tel:`을 사용한 전화번호
- `mailto:`를 사용한 이메일 주소
- 브라우저에서 기본으로 지원하지 않는 기타 스킴에 대해서, 웹사이트가 [`navigator.registerProtocolHandler()`](https://developer.mozilla.org/ko/docs/Web/API/Navigator/registerProtocolHandler)를 사용해 자신을 스킴에 연결할 수 있습니다.
...
`href`로 연결한 링크의 인간 언어를 지정합니다. 별도의 기능은 없습니다. 전역 <HTMLAttr>lang</HTMLAttr> 특성과 같은 값을
사용합니다.
...
공백으로 구분하는 URL 리스트입니다. 사용자가 링크를 따라가면 본문 `PING`으로 하는 [`POST`](https://developer.mozilla.org/ko/docs/Web/HTTP/Methods/POST) 요청을 주어진 URL 모두에 전송합니다. 보통 행동 추적 용도로 사용합니다.
...
- `no-referrer-when-downgrade`: TLS(HTTPS) 지원을 하지 않는 출처에 대해서는 `Referer` 헤더를 전송하지 않습니다. `referrerpolicy` 특성을 따로 지정하지 않은 경우 기본 정책으로 사용합니다.
...
연결한 URL과 현재 문서 사이의 관계를 명시합니다. 공백으로 구분한 [링크 유형 값](https://developer.mozilla.org/en-US/docs/Web/HTML/Link_types)의 목록이어야
합니다.
...
연결한 URL을 표시할 위치입니다. 가능한 값은 브라우징 맥락(탭, 창, <code>&lt;iframe&gt;</code>)의 이름입니다.
...
`target="_blank"`는 `rel="noopener"`를 암시합니다. 따라서 `window.opener`를 설정하지 않습니다. [브라우저 호환성](#브라우저-호환성)에서 이 동작의 지원 범위를 확인하세요.
...
[HTML 명세](https://html.spec.whatwg.org/multipage/browsing-the-web.html#scroll-to-the-fragment-identifier)에 나와있는 것과 같이, `href="#top"`"이나 `href="#"`을 사용하면 페이지 최상단으로 이동하는 링크를 만들 수 있습니다.
