---
title: 전역 상태 관리는 그만, SWR을 써보자
excerpt: 전역 상태 "게 섯거라"... Vercel의 SWR, 쉽고 간편하게 서버 데이터를 가져오다
---
...
그러면 React에서는 데이터를 어떻게 관리하고 사용해야 할까?
...
- `key`: API의 고유 키. 메서드 다수를 허용하지 않으면 API의 URL 경로 역시 고유하므로 그걸로 지정해도 된다.
...
그러나 이 방법은 최적의 사용자 경험을 제공하지는 않는데, 이전에 API 상태를 가져온 적이 있더라도 다음에 불러오는 중엔 화면에 아무것도 보여주지 않기 때문이다. `/status`가 가벼운 API라고 해도 화면의 깜빡임으로 인해 좋지 않은데, 무거운 API라면 더더욱 stale한 데이터라도 보여주고, 대신 최신 상태가 아니라고 알려주는 편이 나을 것이다. 오류기 빌셍헸을 때도 마찬가지로, 이전 데이터가 존재한다면 영양가 없는 오류 경고만 보여주지 말고 받아온 적 있는걸 보여주는 편이 더 부드러운 UX를 제공하지 않을까?

<Callout color="warn" icon="💡" label="참고">
  무조건 이 방식이 최선인 것은 아닙니다.
</Callout>
...
웹 앱에서 전역 상태를 사용하는 이유 중 작지 않은 부분이 원격 데이터라고 생각하는데, SWR과 로컬 상태만으로도 충분히 가능한 규모라면 정말 이 상태가 전역이어야 하는 것인지 고려해봐야 하지 않을까. 한국에서 가장 많은 점유율을 차지하는 Redux를 단독으로 쓴다는 곳은 보지 못했다.

시간에 따른 상태의 변화를 기록해야 할 때 처럼 [Redux가 **필요**할 수도](https://medium.com/@dan_abramov/you-might-not-need-redux-be46360cf367) 있다. 아니면 버그 리포트에 앱의 상태를 통째로 덤핑해야 해서 MobX가 필요할 수도 있다. SWR은 "상태"를 관리하지 않으므로 Redux와 함께 쓰지 않을 법도 없으나, 굳이 추가해야 할 필요도 없다. 라이브러리가 아무리 좋아보여도 자신에게 맞는지는 충분히 생각해볼 여지가 있다.
