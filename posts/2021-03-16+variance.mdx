...
excerpt: Variance, variance!
image: /images/2021-03-16/variance.png
---
...
const obj = {
  ...
  // 속성 단축 표기법
  bar() { console.log(this) },      // {foo: function, bar: function}
}
```

구분해서 부르자면, `foo`는 `this` 맥락을 생성하지 않으므로 객체의 키에 할당한 함수, `bar`는 `this`를 객체로 묶어주므로 메서드라고 할 수 있을까?

TypeScript에서 객체 타입을 선언할 때도 두 표기법을 모두 사용할 수 있다. 그런데 결과물에는 차이가 없는 것처럼 보여서, 그저 팀의 코드 컨벤션 수준에 그칠 수도 있다.
...
- 제공한 타입을 매개변수로도 사용하고 반환형으로도 사용하는 함수 타입은 무공변(invariance)해서, 기존 관계를 유지하지 않는다.
...
`animalViewer = dogViewer`는 성립하지 않아야 하지만... 타입 시스템은 유효하다고 생각한다. 속성 단축 표기법을 사용할 경우 실제 공변/반변성과는 달리 **양변**(bivariance)한다. `--strict`를 켰음에도 불구하고 타입 시스템의 안정성을 확보할 수 없다!

그러므로 인터페이스건, 객체 타입이건, 정말로 양변하는 타입을 의도한 것이 아니면 항상 화살표 표기법을 사용하는 것이 좋겠다.
